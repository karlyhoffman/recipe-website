...
- [x] T004 [P] Create `supabase/lib/rate-limiter.ts`: `RateLimitEntry` interface (`count: number`, `windowStart: number`); per-IP `Map<string, RateLimitEntry>` and a module-level `globalEntry` for cross-IP limiting; `isRateLimited(ip: string): boolean` (5 per-IP / 25 global attempts per 15-min fixed window — either triggers 429); `recordFailedAttempt(ip: string): void` (increments both counters); `clearAttempts(ip: string): void` (clears only the per-IP entry; global counter is not reset so distributed-attack accumulation is preserved)
...
- [x] T005 [P] [US1] Create `supabase/app/api/auth/login/route.ts`: `POST` handler; runtime-validate `username` and `password` are strings and non-empty (400 if missing or wrong type); check `isRateLimited(ip)` from `x-forwarded-for` (429 if limited); compare `username` against `ADMIN_USERNAME` env var and `password` against `ADMIN_PASSWORD_HASH` using `bcryptjs.compare` (401 on mismatch, with `recordFailedAttempt`); on success call `clearAttempts`, `signToken`, set `admin_session` cookie via `cookies().set`, validate `returnUrl` (relative path, no `://`, no `//` prefix), return `{ ok: true, redirectTo }` (200); catch-all returns 500 with `"Something went wrong, please try again."`
...
- [x] T007 [US1] Create `supabase/components/LoginForm.tsx`: Client Component (`'use client'`); controlled `username` and `password` inputs; client-side empty-field validation (inline errors, no fetch) satisfying SC-007; POST body sends `username.trim()` (not the raw value) to match server-side comparison; `fetch('/api/auth/login', { method: 'POST', body: JSON.stringify({username: username.trim(), password, returnUrl}) })`; disable submit button and show loading state while fetch is in-flight (prevents double-submit); display API error message on non-2xx; `router.push(redirectTo); router.refresh()` on success (refresh required to force re-render of shared layout so Navbar reflects authenticated state); display session-expiry message ("Your session has expired. Please log in again.") when `expired` prop is `true`; WCAG 2.1 AA: visible `<label>` elements, keyboard navigable, focus management on error (focus moves to first error field on submit failure); inline validation error elements MUST be associated with their input via `aria-describedby` (FR-012); session-expiry message MUST use `role="alert"` so screen readers announce it (FR-012)
...
- [x] T009 [US2] Modify `supabase/proxy.ts`: import `verifyToken` from `lib/session.ts`; remove Supabase `createServerClient` / `auth.getUser()` logic; attempt `verifyToken` from `admin_session` cookie; protected routes are `/import` (exact), `/import/**`, and `/api/import/**` — `startsWith('/import')` is intentionally NOT used as it would over-match unrelated routes; on protected routes: redirect unauthenticated requests to `/login?returnUrl=<original-path>`, expired-JWT requests to `/login?returnUrl=<path>&expired=1`; on `/login`: redirect authenticated requests to `/import`; on `/login` with an expired JWT (and `?expired` not already in URL): redirect to `/login?expired=1` so the expiry banner is shown even when the user navigates directly to the login page; on all requests: set `x-user-authenticated: 'true'` or `'false'` header; on authenticated requests: refresh the `admin_session` cookie with a new 24h expiry (sliding window); pass `/api/auth/**` through without auth check
...
- [x] T011 [P] [US2] Modify `supabase/app/api/import/save/route.ts`: remove the Supabase auth check — the proxy now enforces authentication for this route; always use `createServiceRoleClient()` (the dev/prod branch is no longer needed since there is no Supabase user session)
...
- [x] T013 [US4] Create `supabase/app/api/auth/logout/route.ts`: `POST` handler; delete the `admin_session` cookie by calling `cookies().set('admin_session', '', { ...getCookieOptions(), expires: new Date(0), maxAge: 0 })`; return `NextResponse.redirect('/')` (302) — redirects to homepage, not `/login`; idempotent — no error if cookie is absent
...
- [x] T016 [P] Validate the full quickstart.md flow end-to-end: install deps (T001), generate bcrypt hash, generate JWT secret, set `.env.local`, run `npm run dev`, test login flow (valid creds → /import), test session expiry (delete cookie → redirect), test rate limiting (5 bad passwords → rate-limit error on 6th attempt)
- [x] T017 Add `supabase/migrations/0007_grant_import_recipe_service_role.sql`: `GRANT EXECUTE ON FUNCTION public.import_recipe(text,text,text,text,timestamptz,jsonb,jsonb,integer,integer,integer,text,uuid[]) TO service_role` — the 12-param overload added in migration 0006 inherited no explicit grant; `save/route.ts` now always calls it via `createServiceRoleClient()` so `service_role` must have execute permission
